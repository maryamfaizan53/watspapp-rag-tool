...
| Layer | Technology |
|---|---|
| Backend | FastAPI 0.111, Python 3.11, Motor (MongoDB), Redis, FAISS, sentence-transformers |
...
| Infra | Docker Compose (MongoDB, Redis, Ollama, backend, frontend) |

### Key directories
```
backend/
  app/api/          — FastAPI routers (auth, tenants, documents, metrics, webhooks, health)
  app/models/       — Pydantic/Motor models (7 collections)
  app/services/     — rag.py, ingestion.py, embeddings.py, llm.py, transcription.py, usage_worker.py
  app/db/           — mongo.py, redis.py, faiss_store.py (LRU-cached, per-tenant)
...
| Variable | Notes |
|---|---|
| `MONGODB_URI` | `mongodb://localhost:27017/psx_chatbot` or Atlas URI |
...
- ❌ `patch("app.db.mongo.get_db", ...)` — won't affect already-imported local copies
- ✅ `patch("app.main.connect_db", ...)` — patches lifespan's local import
- ❌ `patch("app.db.mongo.connect_db", ...)` — won't affect main.py's local binding
